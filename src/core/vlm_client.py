...
## 你的任务
# 评分细则（来自比赛规则，提要化）
- 结构与目录完整：必须覆盖作业区概况、QHSE目标指标、岗位设置与职责、重点业务流程及内容、操作指南、应急处置、风险辨识与控制、考核、附录等；
- 编号规范：1、1.1、1.1.1 等层级必须清晰且连续；
- 语言规范：标点、语句通顺、术语统一；
- 图文一致：文字描述与图表/图片标注一致；
- 应急处置专项：步骤完整（发现→上报→评估→启动→现场处置→恢复→总结），顺序合理，关键步骤不可缺失；
...
## 你的任务
# 评分细则（来自比赛规则，提要化）
- 影像图/示意图：管道中心线+左右影响范围（三条平行线）必须明确；建筑物名称/属性/人数标注完整；
- 入场路线：不得穿山、穿墙、穿越建筑、跨河无桥，且应在可行车道路；
- 逃生路线：应向管道两侧，集结点在影响半径外；
- 图文一致：文字与图例、表格数据相互一致（如区段长、桩号、建筑信息）；
- 数据合规：电位值范围、风险等级术语（低/中/较高/高）必须规范；
检查文档和图片，找出以下问题：
